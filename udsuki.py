...
if "雪" in gt.weather and "雨か雪" not in gt.weather:
    c_text = "今日は雪が降る予報になっています。\
交通機関の情報にも注意して出かけてくださいね！"

#ただの「晴れ」の場合
elif "100" in gt.weather_id:
    c_text = "今日はよく晴れたいい天気になりそうで、\
私も頑張れそうです！\n皆さんも良い1日を〜♪"

#強い雨が降る場合
elif "307" in gt.weather_id or "308" in gt.weather_id:
    c_text = "今日は強い雨と風が吹き荒れるようです。\
出かける際は十分に気をつけてくださいね！"
...
傘を忘れずに持っていきましょう".format(min(gt.rain[1],gt.rain[2],gt.rain[3]))
        ...
        c_text = "今日は午前中の降水確率が{}％、午後は{}％となる見込みです。\
傘を忘れずに持っていきましょう。".format(gt.rain[1],gt.rain[2])
    elif min(gt.rain[2],gt.rain[3]) >= 30:
        c_text = "今日は午後の降水確率が{}％、夜は{}％となっています。朝は雨が降って\
いなくても、傘を持っていくといいでしょう。".format(gt.rain[2],gt.rain[3])
    elif gt.rain[3] >= 30:
        c_text = "今日は夜の降水確率が{}％となっていて、夕方から雨が降りそうです。\
帰りが遅い人は傘を持っていくといいでしょう。".format(gt.rain[3])
    elif gt.rain[2] >= 30:
