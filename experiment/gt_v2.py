    ...
    def __init__(self,url,p_id,place):
        ...
        for youso in self.area:
            ...
            for i in range(len(self.reporting_date_array)):
                self.gt_box_array.append([self.place,self.reporting_date_array[i],self.weather_array[i],self.weather_id_array[i],self.kion_box[i],self.jikantai[i],self.rain[i]])
